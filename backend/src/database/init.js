...
const initData = async () => {
  ...
  try {
    ...
    if (bikes[0].count === 0) {
      const [stationList] = await connection.query(
        "SELECT id, total_docks, available_bikes FROM stations ORDER BY id",
      );
    ...
    const [rides] = await connection.query(
      "SELECT COUNT(*) as count FROM ride_records",
    );
    if (rides[0].count === 0) {
      const [bikeList] = await connection.query(
        "SELECT id FROM bikes LIMIT 50",
      );
      const [stationList] = await connection.query(
    ...
    if (faults[0].count === 0) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const status = statuses[Math.floor(Math.random() * statuses.length)];
        const reportTime = dayjs().subtract(
        ...
        await connection.query(
          ...
          [
            ...
            `${faultType}故障，需要维修`,
            phone,
            reportTime.format("YYYY-MM-DD HH:mm:ss"),
            status,
            maintenanceStaff,
            maintenanceResult,
            maintenanceTime,
    ...
    if (tasks[0].count === 0) {
      const [stationList] = await connection.query("SELECT id FROM stations");
      ...
      for (let i = 0; i < 8; i++) {
        ...
        if (status === "执行中" || status === "已完成") {
          actualStart = scheduledTime
            .subtract(1, "hour")
            .format("YYYY-MM-DD HH:mm:ss");
        }
        if (status === "已完成") {
          actualEnd = scheduledTime
            .add(1, "hour")
            .format("YYYY-MM-DD HH:mm:ss");
        }
        ...
        await connection.query(
          ...
          [
            taskCode,
            stationList[fromIdx].id,
            stationList[toIdx].id,
            bikeCount,
            plate,
