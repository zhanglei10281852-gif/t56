...
router.get("/reports", authMiddleware, async (ctx) => {
  ...
  if (user.role === "dispatcher" && user.district) {
    whereClause +=
      " AND f.bike_id IN (SELECT id FROM bikes WHERE station_id IN (SELECT id FROM stations WHERE district = ?))";
...
router.post("/reports", authMiddleware, async (ctx) => {
  const { bike_code, fault_type, description, reporter_phone } =
    ctx.request.body;
  ...
  try {
    ...
    const [bikes] = await connection.query(
      "SELECT * FROM bikes WHERE bike_code = ?",
      [bike_code],
    );
    if (bikes.length === 0) {
      ctx.status = 404;
    ...
    await connection.query("UPDATE bikes SET status = '维修中' WHERE id = ?", [
      bike.id,
    ]);
