...
router.get("/", authMiddleware, async (ctx) => {
  ...
  const stationsWithRate = stations.map((s) => ({
    ...
    fillRate:
      s.total_docks > 0
        ? ((s.available_bikes / s.total_docks) * 100).toFixed(1)
        : 0,
    statusLevel:
      s.total_docks > 0
        ? s.available_bikes / s.total_docks > 0.85
          ? "surplus"
          : s.available_bikes / s.total_docks < 0.15
            ? "shortage"
            : "normal"
        : "normal",
  }));
...
router.get("/:id", authMiddleware, async (ctx) => {
  ...
  ctx.body = {
    ...station,
    fillRate:
      station.total_docks > 0
        ? ((station.available_bikes / station.total_docks) * 100).toFixed(1)
        : 0,
    bikes,
  };
});
...
router.put("/:id", authMiddleware, async (ctx) => {
  ...
  if (station_name !== undefined) {
    ...
    updateParams.push(station_name);
  }
  ...
  if (available_bikes !== undefined) {
    const currentTotal =
      total_docks !== undefined ? total_docks : stations[0].total_docks;
...
router.delete("/:id", authMiddleware, async (ctx) => {
  ...
  const [bikes] = await pool.query(
    "SELECT COUNT(*) as count FROM bikes WHERE station_id = ?",
    [id],
  );
  if (bikes[0].count > 0) {
    ctx.status = 400;
