...
router.get('/', authMiddleware, async (ctx) => {
  const { station_id, status, bike_type, page = 1, pageSize = 20, keyword } = ctx.query;
  ...
  if (user.role === 'dispatcher' && user.district) {
    whereClause += ' AND b.station_id IN (SELECT id FROM stations WHERE district = ?)';
...
router.post('/', authMiddleware, async (ctx) => {
  ...
  const { bike_code, bike_type = '普通', status = '在桩', station_id } = ctx.request.body;
  ...
  const [existing] = await pool.execute('SELECT id FROM bikes WHERE bike_code = ?', [bike_code]);
  ...
  if (station_id) {
    const [stations] = await pool.execute('SELECT * FROM stations WHERE id = ?', [station_id]);
...
router.put('/:id', authMiddleware, async (ctx) => {
  ...
  const { id } = ctx.params;
  const { bike_type, status, station_id, last_maintenance_date } = ctx.request.body;

  const [bikes] = await pool.execute('SELECT * FROM bikes WHERE id = ?', [id]);
  if (bikes.length === 0) {
    ctx.status = 404;
    ctx.body = { code: 404, message: '车辆不存在' };
    return;
  ...
  try {
    ...
    if (station_id !== undefined && station_id !== oldStationId) {
      ...
      if (station_id && status === '在桩') {
        const [stations] = await connection.execute('SELECT * FROM stations WHERE id = ?', [station_id]);
    ...
    if (bike_type !== undefined) { updateFields.push('bike_type = ?'); updateParams.push(bike_type); }
    if (status !== undefined) { updateFields.push('status = ?'); updateParams.push(status); }
    if (station_id !== undefined) { updateFields.push('station_id = ?'); updateParams.push(station_id || null); }
    if (last_maintenance_date !== undefined) { updateFields.push('last_maintenance_date = ?'); updateParams.push(last_maintenance_date); }
...
router.delete('/:id', authMiddleware, async (ctx) => {
  ...
  try {
    ...
    await connection.execute('DELETE FROM bikes WHERE id = ?', [id]);
    await connection.commit();
