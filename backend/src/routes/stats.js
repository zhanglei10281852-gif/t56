...
router.get('/overview', authMiddleware, async (ctx) => {
  ...
  const [stationStats] = await pool.execute(
  ...
  const bikeWhere = user.role === 'dispatcher' && user.district
    ? 'WHERE station_id IN (SELECT id FROM stations WHERE district = ?)'
    : '';
  const bikeParams = user.role === 'dispatcher' && user.district ? [user.district] : [];

  const [bikeStats] = await pool.execute(
  ...
  const rideWhere = user.role === 'dispatcher' && user.district
    ? 'WHERE DATE(start_time) = ? AND start_station_id IN (SELECT id FROM stations WHERE district = ?)'
    : 'WHERE DATE(start_time) = ?';
  const rideParams = user.role === 'dispatcher' && user.district ? [today, user.district] : [today];

  const [todayRideStats] = await pool.execute(
  ...
  const dispatchWhere = user.role === 'dispatcher' && user.district
    ? 'WHERE DATE(created_at) = ? AND from_station_id IN (SELECT id FROM stations WHERE district = ?)'
    : 'WHERE DATE(created_at) = ?';
  const dispatchParams = user.role === 'dispatcher' && user.district ? [today, user.district] : [today];

  const [todayDispatchStats] = await pool.execute(
...
router.get('/rides/hourly', authMiddleware, async (ctx) => {
  ...
  if (user.role === 'dispatcher' && user.district) {
    whereClause += ' AND start_station_id IN (SELECT id FROM stations WHERE district = ?)';
    params.push(user.district);
  }
...
router.get('/rides/stations', authMiddleware, async (ctx) => {
  ...
  if (user.role === 'dispatcher' && user.district) {
    whereClause = 'WHERE station_id IN (SELECT id FROM stations WHERE district = ?)';
    params.push(user.district);
  }

  const stationField = type === 'return' ? 'end_station_id' : 'start_station_id';
...
router.get('/dispatch/completion-rate', authMiddleware, async (ctx) => {
  ...
  if (user.role === 'dispatcher' && user.district) {
    whereClause += ' AND from_station_id IN (SELECT id FROM stations WHERE district = ?)';
    params.push(user.district);
  }
  ...
  const data = results.map(r => ({
    ...
    rate: r.total_tasks > 0 ? ((r.completed_tasks / r.total_tasks) * 100).toFixed(1) : 0
  }));
...
router.get('/fault/types', authMiddleware, async (ctx) => {
  ...
  if (user.role === 'dispatcher' && user.district) {
    whereClause += ' AND bike_id IN (SELECT id FROM bikes WHERE station_id IN (SELECT id FROM stations WHERE district = ?))';
    params.push(user.district);
  }
