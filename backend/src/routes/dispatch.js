...
router.post("/tasks", authMiddleware, async (ctx) => {
  ...
  try {
    ...
    const [fromStations] = await connection.query(
    ...
    const [toStations] = await connection.query(
      "SELECT * FROM stations WHERE id = ?",
      [to_station_id],
    );
    ...
    if (user.role === "dispatcher") {
      if (
        fromStations[0].district !== user.district &&
        toStations[0].district !== user.district
      ) {
        ctx.status = 403;
    ...
    if (fromStations[0].available_bikes < bike_count) {
      ...
      ctx.body = {
        code: 400,
        message: `调出站点可用车辆不足，当前仅有${fromStations[0].available_bikes}辆`,
      };
      await connection.rollback();
      return;
    }
    ...
    if (toStations[0].empty_docks < bike_count) {
      ...
      ctx.body = {
        code: 400,
        message: `调入站点空桩不足，当前仅有${toStations[0].empty_docks}个空桩`,
      };
      await connection.rollback();
      return;
    }
...
router.put("/tasks/:id/status", authMiddleware, async (ctx) => {
  ...
  try {
    ...
    const [tasks] = await connection.query(
      "SELECT * FROM dispatch_tasks WHERE id = ? FOR UPDATE",
      [id],
    );
    if (tasks.length === 0) {
      ctx.status = 404;
    ...
    if (user.role === "dispatcher") {
      const [fromStation] = await connection.query(
        "SELECT district FROM stations WHERE id = ?",
    ...
    if (status === "已完成") {
      ...
      const [bikesToMove] = await connection.query(
...
router.delete("/tasks/:id", authMiddleware, async (ctx) => {
  ...
  if (user.role === "dispatcher") {
    const [fromStation] = await pool.query(
      "SELECT district FROM stations WHERE id = ?",
      [tasks[0].from_station_id],
    );
    if (fromStation[0].district !== user.district) {
      ctx.status = 403;
  ...
  await pool.query("DELETE FROM dispatch_tasks WHERE id = ?", [id]);
