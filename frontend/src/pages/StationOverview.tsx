...
import { Row, Col, Card, Tabs, Tag, Statistic, Space, Button, Badge, Typography } from 'antd'
import { ThunderboltOutlined, CarOutlined, WarningOutlined, ReloadOutlined } from '@ant-design/icons'
import { getStationList, getDispatchSuggestions, Station, DispatchSuggestionsResult } from '@/api/stations'
...
const StationCard = ({ station }: StationCardProps) => {
  ...
  const statusColor = station.statusLevel === 'surplus' ? 'red' : station.statusLevel === 'shortage' ? 'gold' : 'green'
  const statusText = station.statusLevel === 'surplus' ? '车辆过剩' : station.statusLevel === 'shortage' ? '车辆不足' : '正常'

  const docks = Array.from({ length: station.totalDocks }, (_, i) => i < station.availableBikes)
  ...
  return (
    <Card
      ...
      extra={<span style={{ fontSize: 12, color: '#999' }}>{station.stationCode}</span>}
    >
      <div style={{ marginBottom: 8 }}>
        <div className="dock-progress">
          <div
            ...
            style={{
              width: `${fillRate}%`,
              backgroundColor: station.statusLevel === 'surplus' ? '#ff4d4f' : station.statusLevel === 'shortage' ? '#faad14' : '#52c41a'
            }}
          />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#666' }}>
          <span>可用: {station.availableBikes}</span>
          <span style={{ fontWeight: 600, color: statusColor }}>{fillRate.toFixed(1)}%</span>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
        ...
        {station.totalDocks > 40 && <span style={{ fontSize: 10, color: '#999' }}>...</span>}
      </div>
...
const StationOverview = () => {
  ...
  const [suggestions, setSuggestions] = useState<DispatchSuggestionsResult | null>(null)
  ...
  const filteredStations = activeTab === 'all'
    ? stations
    : stations.filter(s => s.district === activeTab)

  const tabItems = [
    { key: 'all', label: '全部站点' },
    ...(userInfo?.role === 'admin' ? districts.slice(1).map(d => ({ key: d, label: d })) : [])
  ]
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={6}>
          <Card>
            <Statistic
              title="站点总数"
              value={overview?.stations.total_stations || 0}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="车辆总数"
              value={overview?.bikes.total_bikes || 0}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="今日骑行"
              value={overview?.todayRides.today_rides || 0}
