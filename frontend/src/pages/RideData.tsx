...
import { Row, Col, Card, Table, Statistic, Tag, DatePicker, Typography } from 'antd'
import { ThunderboltOutlined, UserOutlined, CarOutlined } from '@ant-design/icons'
import ReactECharts from 'echarts-for-react'
import { getRideRecords, RideRecord } from '@/api/ride'
import { getHourlyRides, getStationRideRank, HourlyRideData, StationRideRank, OverviewStats, getOverviewStats } from '@/api/stats'
...
const RideData = () => {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [recordsRes, hourlyRes, borrowRes, returnRes, overviewRes] = await Promise.all([
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="今日骑行人数"
              value={overview?.todayRides.today_users || 0}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="骑行中车辆"
              value={overview?.bikes.riding_bikes || 0}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="今日调度任务"
              value={overview?.todayDispatches.today_dispatches || 0}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={12}>
          <div className="chart-container">
            ...
            <ReactECharts option={borrowRankChartOption} style={{ height: 320 }} />
          </div>
        </Col>
        <Col span={12}>
          <div className="chart-container">
            <div className="chart-title">热门还车站点 TOP 10</div>
            <ReactECharts option={returnRankChartOption} style={{ height: 320 }} />
          </div>
        </Col>
      </Row>
