...
const Statistics = () => {
  ...
  const loadData = async () => {
    try {
      const [districtRes, fillRateRes, completionRes, faultTypeRes, dailyRes] =
        await Promise.all([
    ...
    series: [
      {
        ...
        type: "bar",
        data:
          fillRateData?.distribution.map((d, i) => ({
            value: d.count,
            itemStyle: {
              color:
                i <= 1
